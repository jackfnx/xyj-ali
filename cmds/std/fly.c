...
int main(object me, string arg)
{
    ...
    if (arg=="stone") loc="/d/4world/entrance";
        ...
        if (!wizardp(me)
        &&  (!objectp(map = present("eastsea map", me)) || (string)map->query("unit") != "张")) {
            tell_object(me, "你在天上转了半天也不知该往那边飞。。。\n");
            message_vision("\n$N失望地从云上跳了下来！\n", me);
            return 1;
        }
//        write("你驾着祥云向东边海中飞去。。。\n\n");
        ...
        if (!wizardp(me)
        &&  (!objectp(map = present("xueshan map", me)) || (string)map->query("unit") != "张")) {
            tell_object(me, "你在天上转了半天也不知该往那边飞。。。\n");
            message_vision("\n$N失望地从云上跳了下来！\n", me);
            return 1;
        }
