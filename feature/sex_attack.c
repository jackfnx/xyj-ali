...
string *query_makelover() { return makelover; }
...
varargs int is_makeinglove(string id)
{
    if (!id) return sizeof(makelover) > 0;

    return member_array(id, makelover)!=-1;
}
...
void makelove_ob(object ob)
{
    if (!ob) return;

    if ( member_array(ob->query("id"), makelover)==-1 )
        makelover += ({ ob->query("id") });
...
{
    string penis, pussy;
    object attacker, sufferer;

    if (!ob) return;

    if ( member_array(ob->query("id"), fucker)==-1 ) {
        fucker += ({ ob->query("id") });
        "/adm/daemons/sexd"->plug_penis(this_object(), ob);
    }
...
int remove_makelover(object ob)
{
    if (is_makeinglove(ob->query("id"))) {
        makelover -= ({ ob->query("id") });
        remove_enemy(ob);
        return 1;
    }
...
int remove_fucker(object ob)
{
    if (is_fucking(ob->query("id"))) {
        fucker -= ({ ob->query("id") });
        "/adm/daemons/sexd"->draw_penis(this_object(), ob);
        return 1;
    }
}
...
int attack()
{
    ...
    if (objectp(opponent)) {
        ...
        if (!is_makeinglove(opponent->query("id"))) {
            COMBAT_D->fight(this_object(), opponent);
        } else {
            "/adm/daemons/sexd"->makelove(this_object(), opponent);
        }
        return 1;
    } else
        return 0;
