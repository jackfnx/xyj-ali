...
void hurt(object ob, object me)
{
    ...
    if (!random(10)) {
        command("surrender");
        command("say 玩玩而已，干嘛下手这么重？算你厉害行不行？不和你打了！");
    ...
    switch (random(6)) {
        case 0: command("pain"); break;
        case 1: command("pain "+me->query("id")); break;
        case 2: command("angry"); break;
        case 3: command("kick"); break;
        case 4: command("slogan "+me->query("id")); break;  
        case 5: command("sign2"); break;
    }
} 
...
int do_stop()
{
    ...
    if (flower->move(target)) {
        ...
        target->command_function("norose");
        command(":P");
    }
