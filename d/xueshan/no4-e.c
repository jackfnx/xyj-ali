...
雪岭的尽头有一个小小的洞(cave)，据说里面有一只赤练小蛇，吃
了后可以大补，但怎么把它从洞里引出来，可就没几个人知道了。
LONG);

    set("item_desc", ([
        "cave" : "一个小洞，里面黑黑的，什么都看不见。\n", 
    ]));
...
int come_she()
{
    ...
    if (douzi && douzi->query("value") == 20000) {
        tell_room(where, "一只小蛇从洞里爬了出来，将"+ douzi->name() +"叼在嘴里。\n");
...
int back_she()
{
    ...
    if (she) {
        tell_room(where, she->name() + "将豆子一口吞了下去，然后机警的爬回洞里。\n");
        destruct(she);
