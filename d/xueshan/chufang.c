...
这里是厨房。雪山弟子性格凶残者极多，常将所获之物生吞活剥。
所以这里经常不开灶，屋子角落处摆着一口大铁锅(guo)，是雪山
弟子作汤用的，旁边还有一个白瓷盆(pen)。有个怪头怪脑的小妖
正在里面懒洋洋的躺着。他腰上挂着一块牌子，上面歪歪拐拐地
写着：『小钻风』。
LONG);
...
int do_add(string arg)
{
    ...
    if (sscanf(arg, "%s in guo", item_name) != 1)
        return notify_fail("用法：add <sth> in guo。\n");
    ...
    if (!(item = present(item_name, who))) return notify_fail("你身上没有这东西。\n");
