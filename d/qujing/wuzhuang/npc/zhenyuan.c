...
string expell_me()
{
    ...
    if((string)me->query("family/family_name") == "五庄观") {
        message_vision("镇元大仙道：你既然要走，那也不能强留。但镇元神功是五庄观不传之秘，可不能带出观外。\n", me);
...
string asked_guo()
{
   ...
   if (total > 3) return ("哎呀！这么多人，现在只有三枚成熟的人参果。\n");
        ...
        for (i = 0; i < sizeof(inv); i++) {
            if (userp(inv[i]) && (!wizardp(inv[i]))) {//wiz not counted...
...
void inviting(object me)
{
    ...
    if (!me->query_temp("invite")) return;
