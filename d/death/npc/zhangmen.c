...
"好！师兄有意出任大力鬼王一职？我们就切磋一下吧！\n";
        else return "好！师姊有意出任大力鬼王一职？我们就切磋一下吧！\n";
}
...
int accept_fight(object ob)
{
        ...
        {
                command ("say 失陪了！\n");
                return notify_fail("这位大力鬼王尚在比武之中，请稍候。\n");
        }

        if (ob->query_temp("fangcun_zm_applied"))
        {
                command("say 好！尚请未来的大力鬼王手下留情。\n");
        }
        else
        {
...
ob->query_temp("fangcun_zm_applied"))
                {
                        message_vision
...
CHANNEL_D->do_channel(who,"chat","大力鬼王"+ob->query("name")+"今日走马上任。恭请各位仙长多加捧场！");
