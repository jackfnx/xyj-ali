...
void create ()
{
    set_name("〖法宝炼制指南〗", ({"fabao guide", "guide", "blueprint", "map", "book"}));
    set_weight(100);
    seteuid(getuid());
    if (clonep())
...
int do_make()
{
    ...
    if ( me->query("combat_exp") < 20000 )
        return notify_fail("你的道行不够，不能炼制法宝。\n");
    
    if ( me->query("max_force") < 300 )
        return notify_fail("你的内力不够，不能炼制法宝。\n");
        
    if ( me->query("max_mana") < 300 )
        return notify_fail("你的法力不够，不能炼制法宝。\n");
    
    if ( me->query("force") < (me->query("max_force")+100) )
        return notify_fail("你的真气不足，不能炼制法宝。\n");
...
int do_seal(string arg)
{
    ...
    if (equipped = fabao_ob->query("equipped"))
        fabao_ob->unequip();
    fabao_ob->set("seal", 1);
    fabao_ob->set("short", fabao_ob->short() + CYN + "『封印中』" + NOR);
    fabao_ob->set("weapon_prop/damage", 1);
    tell_object(this_player(), fabao_ob->name() + "上泛起了一层淡淡的光，那种毁灭的气息消失了。\n");
    fabao_ob->save();
    if (equipped)
        fabao_ob->wield();
    return 1;
...
int do_unseal(string arg)
{
    ...
    tell_object(this_player(), fabao_ob->name() + "上放射出耀眼的光芒，一股毁灭的气息扑面而来。\n");
    fabao_ob->save();
    if (equipped)
        fabao_ob->wield();
    return 1;
